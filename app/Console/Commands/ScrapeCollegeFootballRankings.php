            ...
            if (!empty($matches[1])) {
                foreach ($matches[1] as $index => $scrapedTeam) {
                    ...
                    $team = CollegeFootballTeam::where('school', $scrapedTeam)
                        ->orWhere('alt_name1', $scrapedTeam)
                        ->orWhere('alt_name2', $scrapedTeam)
                        ->orWhere('alt_name3', $scrapedTeam)
                        ->first();
                    ...
                    if (!$team) {
                        $team = CollegeFootballTeam::where(function ($query) use ($scrapedTeam) {
                            $query->where('school', 'LIKE', "%{$scrapedTeam}%")
                                ->orWhere('alt_name1', 'LIKE', "%{$scrapedTeam}%")
                                ->orWhere('alt_name2', 'LIKE', "%{$scrapedTeam}%")
                                ->orWhere('alt_name3', 'LIKE', "%{$scrapedTeam}%");
                        })->first();
                    }
