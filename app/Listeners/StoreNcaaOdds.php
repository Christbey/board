...
class StoreNcaaOdds
{
    ...
    {
        Log::info('StoreNcaaOdds listener triggered.');

        foreach ($event->odds as $eventData) {
            $homeTeam = NcaaTeam::firstOrCreate(['name' => $eventData['home_team']]);
            $awayTeam = NcaaTeam::firstOrCreate(['name' => $eventData['away_team']]);
            ...
            foreach ($eventData['bookmakers'] as $bookmaker) {
                ...
                $existingOdds = NcaaOdds::where('event_id', $data['event_id'])
    ...
    private function prepareOddsData($event, $bookmaker, $homeTeam, $awayTeam)
    {
        ...
        $data = [
            ...
            'is_live' => $commenceTime <= Carbon::now()->setTimezone('America/Chicago')->format('Y-m-d H:i:s'),
        ];
