...
class StoreNflOdds
{
    public function handle(OddsFetched $event)
    {
        ...
        foreach ($event->odds as $eventData) {
            $homeTeam = NflTeam::firstOrCreate(['name' => $eventData['home_team']]);
            $awayTeam = NflTeam::firstOrCreate(['name' => $eventData['away_team']]);
            ...
            foreach ($eventData['bookmakers'] as $bookmaker) {
                ...
                $existingOdds = NflOdds::where('event_id', $data['event_id'])
